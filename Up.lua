...
function Library:CreateWindow(config)
	...
	MinButton.MouseButton1Click:Connect(function()
		...
		if minimized then
			...
			-- Hide TabContainer and ContentContainer when minimized
			TabContainer.Visible = false
			ContentContainer.Visible = false
		else
			Tween(MinButton, {BackgroundTransparency = 1}, 0.2)
			Tween(MainFrame, {Size = UDim2.new(0, 650, 0, 450)}, 0.3, Enum.EasingStyle.Quint)
			-- Show TabContainer and ContentContainer when expanded
	...
	function Window:Tab(name, index)
		...
		TabButton.Name = name
		TabButton.Size = UDim2.new(1, 0, 0, 32)  -- Changed from 38 to 32
		TabButton.BackgroundColor3 = currentTheme.Tertiary
		TabButton.BorderSizePixel = 0
		TabButton.Text = name
		TabButton.TextColor3 = currentTheme.Text
		TabButton.Font = Enum.Font.GothamSemibold
		TabButton.TextSize = 13  -- Slightly smaller text
	...
	if misc then
		task.spawn(function()
			...
			ThanksText.Text = "Thanks for using Nexus UI\nNexus is a fast and lightweight UI design by Claude"
